...
public class SheepWalkF : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(HayplateIn.SheepEat1 == true)
        {
            transform.position = Vector3.Lerp(transform.position, EatPoint.transform.position, speed);
            transform.localEulerAngles = new Vector3(0, 320, 0);
            walkanim = false;
        ...
        if(EatTime == true)
        {
            ...
            if(timeEat <= 0)
            {
                ...
                HayplateIn.SheepEat1 = false;
            }
        }
        if(EatTime == false)
        {
            speed = 0.007f;
            HeadAnim.animeat = false;
            timeEat = 5;
        }
